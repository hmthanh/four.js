...
const KEYBOARD_MOVE_SPEED = Math.PI * 20
...
export class OrbitControls {
  ...
  zoom(scale: number): void {
    ...
    vec3.scale(this._camera.position, this._camera.position, scale * (Math.min(this.maxRadius, Math.max(this.minRadius, radius * scale)) / (radius * scale)))
  ...
  orbit(deltaX: number, deltaY: number): void {
    ...
    const theta = Math.min(this.maxTheta, Math.max(this.minTheta, Math.atan2(offset[2], offset[0]) + deltaTheta)) || Number.EPSILON
